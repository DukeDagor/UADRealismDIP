    ...
    internal class Patch_Ui
    {
        ...
        internal static void Postfix_Update(Ui __instance)
        {
            ...
            if (GameManager.IsWorldMap
                && !Patch_CampaignController.isLoadingNewTurn
                && !ModUtils.NearlyEqual(pastPosChecked, CampaignMap.Instance.lastMousePosition, 0.1f, 0.1f, 0.1f))
            {
                pastPosChecked = CampaignMap.Instance.lastMousePosition;
                BoxCollider bc = null;

                foreach (var provLabel in CampaignMap.Instance.UIMap.ProvinceLabels)
                {
                    if (G.ui.ProvincePopupElement.Root.active
                        && G.ui.ProvincePopupElement.shownFor != provLabel.Id)
                    {
                        continue;
                    }

                    bc = provLabel.GetComponent<BoxCollider>();

                    bool contains = ModUtils.Bounds2DContains(bc.bounds, CampaignMap.Instance.lastMousePosition);

                    if (contains && !G.ui.ProvincePopupElement.Root.active)
                    {
                        // Melon<TweaksAndFixes>.Logger.Msg($"Show: {provLabel.Name}");
                        Patch_CampaignProvincePopupUI.allowNextMouseOver = true;
                        provLabel.OnMouseOver();
                        break;
                    }

                    else if (!contains && G.ui.ProvincePopupElement.Root.active)
                    {
                        // Melon<TweaksAndFixes>.Logger.Msg($"Hide: {provLabel.Name}");
                        Patch_CampaignProvincePopupUI.allowNextMouseExit = true;
                        provLabel.OnMouseExit();
                        break;
                    }
                }

                foreach (var areaLabel in CampaignMap.Instance.UIMap.AreaLabels)
                {
                    if (G.ui.AreaPopupElement.Root.active
                        && G.ui.AreaPopupElement.shownFor != areaLabel.Id)
                    {
                        continue;
                    }

                    bc = areaLabel.GetComponent<BoxCollider>();

                    bool contains = ModUtils.Bounds2DContains(bc.bounds, CampaignMap.Instance.lastMousePosition);

                    if (contains && !G.ui.AreaPopupElement.Root.active)
                    {
                        // Melon<TweaksAndFixes>.Logger.Msg($"Show: {areaLabel.Name}");
                        Patch_CampaignAreaPopupUI.allowNextMouseOver = true;
                        areaLabel.OnMouseOver();
                        break;
                    }

                    else if (!contains && G.ui.AreaPopupElement.Root.active)
                    {
                        // Melon<TweaksAndFixes>.Logger.Msg($"Hide: {areaLabel.Name}");
                        Patch_CampaignAreaPopupUI.allowNextMouseExit = true;
                        areaLabel.OnMouseExit();
                        break;
                    }
                }
            }
